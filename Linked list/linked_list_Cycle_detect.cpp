...
//remove cycle dont work it gives wrong answer but cycle detection work
...
//this even works for circular linked list this is bruteforce approach
void detect_and_removal_of_cycle(node *head)
{
    if(detetcycle(head))
    {
        node *slow=head;
        ...
        while(fast!=NULL and fast->next!=NULL)
        {
            fast=fast->next->next;
            slow=slow->next;
          //  cout<<"Fast->data="<<fast->data<<" slow->data="<<slow->data<<endl;
            if(slow==fast)
            {
                cout<<slow->data<<endl;
                break;
...
{
    node *head;
    cout<<"Enter the number in linkedlist terminated by -1"<<endl;
    cin>>head;
    node*last_node=head;
    ...
    detect_and_removal_of_cycle(head);
    cout<<head<<endl;
}