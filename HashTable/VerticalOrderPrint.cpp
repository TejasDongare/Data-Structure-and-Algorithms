...
node* PreorderInorder(int in[],int pre[],int s,int e)
{
    static int i=0;
    if(s>e)
    {
    ...
    i++;
    temp->left=PreorderInorder(in,pre,s,j-1);
    temp->right=PreorderInorder(in,pre,j+1,e);
    return temp;
}
void Vertical_print(node *head,int d,map<int,vector<int>> &t)
{
    if(head==NULL)
    {
        return;
    }
    t[d].push_back(head->data);
    Vertical_print(head->left,d-1,t);
    Vertical_print(head->right,d+1,t);
}
int main()
{
    node *head=build_tree();
    map<int,vector<int>> t;
    Vertical_print(head,0,t);
    for(auto x:t)
    {
        cout<<x.first<<" -> ";
        for(int v:x.second)
